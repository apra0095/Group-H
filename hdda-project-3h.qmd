...
Furthermore @tbl-factor-market-cor gives us further check using the latent factor model. The result is consistent with @tbl-q2-cor because Factor 1 is also very strongly related to Market Return, with a correlation of 0.9865. This confirms that the main hidden pattern in the stock returns is the broad market movement. The other factors are less related to Market Return, but Factor 2 and Factor 3 have moderate relationships with HML, suggesting that value related effects may explain some of the remaining common variation. Overall, both PCA and the latent factor model show the same message, market Return explains the dominant common movement, while SMB and HML only explain smaller secondary patterns.
